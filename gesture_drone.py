"""
Gesture-controlled drone (ArduPilot / MAVLink).
...
then run this. ESC quits and lands the drone on the way out.
"""
...
YAW_COMMANDS = {"hover", "forward", "back", "left", "right", "ascend", "descend"}
...
class GestureClassifier:
    ...
    def yaw(self, lm):
        ...
        offset = self.tilt(lm) + 90

     
        offset = (offset + 180) % 360 - 180

        if abs(offset) < TILT_DEADZONE:
            return 0

        span = TILT_FULL - TILT_DEADZONE
        strength = min(abs(offset) - TILT_DEADZONE, span) / span
        return int(math.copysign(strength * YAW_SPEED, offset))
...
class DroneWorker:
    ...
    def set(self, command, yaw=0):
        """Called from the camera loop. Returns instantly."""
        with self._lock:
            self._wanted = (command, yaw)
    ...
    def _loop(self):
        while self._running:
            ...
            if wanted is None or wanted[0] is None:
                time.sleep(0.02)
                continue

            command, yaw = wanted
            try:
                self.link.send(command, yaw)
...
class LandmarkApp:
    ...
    def _handle_gesture(self, gesture, yaw):
        ...
        if command not in YAW_COMMANDS:
            yaw = 0                 # no turning mid-takeoff or mid-landing

        self.worker.set(command, yaw)
            ...
            while True:
                ...
                for i, lm in enumerate(result.hand_landmarks):
                    ...
                    if i == 0:
                        ...
                        yaw = self.classifier.yaw(lm)

                self._handle_gesture(gesture, yaw)

                turn = "left" if yaw < 0 else "right" if yaw > 0 else "-"
                cv2.putText(frame, f"CMD: {COMMANDS.get(self.active, '-')}",
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
                            0.7, (255, 255, 0), 2)
                cv2.putText(frame, f"YAW: {turn} ({yaw})",
